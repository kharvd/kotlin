...
import org.jetbrains.jet.codegen.optimization.common.DataFlowGraph
...
class VariableLivenessFrame(val maxLocals: Int) {
    ...
    public fun mergeFrom(other: VariableLivenessFrame) {
        bitSet.or(other.bitSet)     
    }
    
    public fun markAlive(varIndex: Int) {
        bitSet.set(varIndex, true)
    }
    
    public fun markDead(varIndex: Int) {
        bitSet.set(varIndex, false)
    }
    
    public fun isAlive(varIndex: Int): Boolean = bitSet.get(varIndex)
...
fun analyzeLiveness(node: MethodNode): Array<VariableLivenessFrame> {
    val graph = DataFlowGraph.build(node)
